...
for col in NUMERIC_TO_DISCRETIZE
    ...
    df[!, col] = [discretize_val(x) for x in df[!, col]]
end

transactions = [ [ (col in NUMERIC_TO_DISCRETIZE ? string(row[col]) :
    string(col, "_", row[col])) for col in names(df) if !ismissing(row[col]) ]
